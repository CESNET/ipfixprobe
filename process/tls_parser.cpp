...
{
    return handshake->version.major == 3 && handshake->version.minor >= 1 && handshake->version.minor <= 3;
}

bool handshake_has_supported_type(const TLSHandshake *handshake)
{
    return handshake->type == TLS_HANDSHAKE_CLIENT_HELLO || handshake->type == TLS_HANDSHAKE_SERVER_HELLO;
}

bool TLSParser::parse_tls_handshake() noexcept
{
    const auto* handshake = reinterpret_cast<const TLSHandshake*>(m_packet_data + m_header_section_size);
...
{
    const size_t cipher_suite_section_offset = m_header_section_size
...
{
    const size_t compression_methods_section_offset = m_header_section_size
...
void TLSParser::parse_server_names(const uint8_t* extension_data, uint16_t extension_length)
{
    ...
    const uint16_t servername_list_length = ntohs(*reinterpret_cast<const uint16_t*>(extension_data));
    ...
    for (const uint8_t* sni = sni_begin; sni + sizeof(TLSExtensionSNI) <= sni_end;) {
        ...
        m_server_names.emplace_back(reinterpret_cast<const char*>(sni) + sizeof(TLSExtensionSNI), sni_length);
...
void TLSParser::parse_elliptic_curves(const uint8_t* extension_payload, uint16_t extension_length) noexcept
{
    ...
    const uint16_t supported_groups_length = ntohs(*reinterpret_cast<const uint16_t*>(extension_payload));
    ...
    for (const uint8_t* supported_group = supported_groups_begin; supported_group < supported_groups_end;
         supported_group += sizeof(uint16_t)) {
        const uint16_t supported_group_type = ntohs(*reinterpret_cast<const uint16_t*>(supported_group));
...
void TLSParser::parse_alpn(const uint8_t* extension_data, uint16_t extension_length)
{
    ...
    for (const uint8_t* alpn = alpn_begin; alpn + sizeof(uint8_t) <= alpn_end; ) {
        ...
        m_alpns.emplace_back(reinterpret_cast<const char*>(alpn) + sizeof(alpn_length), alpn_length);
...
void TLSParser::parse_supported_versions(const uint8_t* extension_data, uint16_t extension_length) noexcept
{
    ...
    const auto version = reinterpret_cast<const uint16_t*>(extension_data + sizeof(versions_length));
...
bool TLSParser::parse_extensions(const std::function<void(uint16_t, const uint8_t*, uint16_t)>& callable) noexcept{
    ...
    const size_t extensions_section_offset = m_header_section_size
        + sizeof(TLSHandshake) + TLS_RANDOM_BYTES_LENGTH + m_session_id_section_length + m_cipher_suites_section_length
        + m_compression_methods_section_length;
...
static void save_to_buffer(char* destination, const std::vector<std::string_view>& source, uint32_t size, char delimiter) noexcept
{
    std::for_each(source.begin(), source.end(), [destination, write_pos = 0UL, size,delimiter](const std::string_view& alpn) mutable {
...
void TLSParser::parse_quic_user_agent(const uint8_t* extension_payload, uint16_t extension_length) noexcept
{
    ...
    const uint8_t* quic_transport_parameters_end = quic_transport_parameters_begin + extension_length;
    for (const uint8_t* parameter = quic_transport_parameters_begin; parameter < quic_transport_parameters_end; ) {
        size_t offset = 0UL;
        ...
        if (parameter_id == TLS_EXT_GOOGLE_USER_AGENT) {
            m_objects_parsed++;
            m_quic_user_agents.emplace_back(reinterpret_cast<const char*>(parameter + offset), parameter_length);
        }
