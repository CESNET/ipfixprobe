...
void QUIC::quic_get_version   (uint32_t& version_toset)
{
   version_toset = version;
   return;
}
void QUIC::quic_get_sni(char * in)
{
    memcpy(in,sni,BUFF_SIZE);
    return;
}
void QUIC::quic_get_user_agent(char * in)
{
    memcpy(in,user_agent,BUFF_SIZE);
    return;
}

bool quic_check_pointer_pos(uint8_t * current, uint8_t * end)
{
    if (current < end)
        return true;
    return false;
...
uint64_t QUIC::quic_get_variable_length(uint8_t * start , uint64_t &offset)
{
...
void QUIC::quic_get_tls_user_agent(tls_data_struct &data, uint16_t length_ext)
{
   ...
   while (data.data + offset < quic_transport_params_end) {
      ...
      if (param == TLS_EXT_GOOGLE_USER_AGENT) {
         if (length + (size_t) 1 > BUFF_SIZE) {
            length = BUFF_SIZE - 1;
         }
         memcpy(user_agent, data.data + offset, length);
         user_agent[length] = 0; 
...
bool QUIC::quic_is_grease_value_(uint16_t val)
{
...
void QUIC::quic_get_ja3_cipher_suites(std::string &ja3, tls_data_struct &data)
{
   ...
   for (; data.data <= section_end; data.data += sizeof(uint16_t)) {
      ...
      if (!quic_is_grease_value_(type_id)) {
...
bool QUIC::quic_parse_tls_nonext_hdr(tls_data_struct &payload, std::string *ja3)
{
   ...
   if (hs_type == TLS_HANDSHAKE_CLIENT_HELLO) {
      if (ja3) {
         quic_get_ja3_cipher_suites(*ja3, payload);
         if (!payload.valid) {
            return false;
         }
      } else {
...
void QUIC::quic_get_tls_server_name(tls_data_struct &data)
{      
   ...
   while (data.data + sizeof(tls_ext_sni) + offset < list_end) {
      tls_ext_sni *tmp_sni = (tls_ext_sni *) (data.data + offset);
      ...
      if (sni_len + (size_t) 1 > BUFF_SIZE) {
         sni_len = BUFF_SIZE - 1;
      }
      memcpy(sni, data.data + offset, sni_len);
      sni[sni_len] = 0;
      data.sni_parsed++;
      offset += ntohs(tmp_sni->length);
   }
}
bool QUIC::quic_parse_tls()
{

   tls_data_struct payload = {
      (char *) final_payload + quic_crypto_start,
      (char *) final_payload + quic_crypto_start + quic_crypto_len,
      true,
      0,
      0
   };

   if (!quic_parse_tls_nonext_hdr(payload, nullptr)) {
   ...
   while (payload.data + sizeof(quic_ext) <= payload.end) {
      ...
      if (type == TLS_EXT_SERVER_NAME) {
         quic_get_tls_server_name(payload);
         ...
         quic_get_tls_user_agent(payload,length);
...
uint8_t QUIC::quic_draft_version(uint32_t version)
{
...
bool QUIC::quic_check_version(uint32_t version, uint8_t max_version)
{
   uint8_t draft_version = quic_draft_version(version);

   return draft_version && draft_version <= max_version;
}
bool QUIC::quic_obtain_version()
{
...
bool QUIC::quic_derive_secrets(uint8_t *secret)
{
...
bool QUIC::quic_create_initial_secrets()
{
...
bool QUIC::quic_encrypt_sample(uint8_t * plaintext)
{
   int len = 0;
   EVP_CIPHER_CTX *ctx;
...
bool QUIC::quic_decrypt_header()
{
   ...
   */
   ...
   phton64(initial_secrets.iv + sizeof(initial_secrets.iv) - 8, pntoh64(initial_secrets.iv + sizeof(initial_secrets.iv) - 8) ^ packet_number);
   return true;
} // QUICPlugin::quic_decrypt_header
bool QUIC::quic_decrypt_payload()
{
   uint8_t atag[16] = { 0 };
   int len;
...
bool QUIC::quic_check_frame_type(uint8_t* where, FRAME_TYPE frame_type)
{
    return (*where) == frame_type;

}
void QUIC::quic_skip_ack1(uint8_t * start , uint64_t &offset)
{
...
void QUIC::quic_skip_ack2(uint8_t * start , uint64_t &offset)
{
   // https://www.rfc-editor.org/rfc/rfc9000.html#name-ack-frames
   offset++;
   quic_get_variable_length(start,offset);
...
void QUIC::quic_skip_connection_close1(uint8_t * start , uint64_t &offset)
{
...
void QUIC::quic_skip_connection_close2(uint8_t * start , uint64_t &offset)
{
      // https://www.rfc-editor.org/rfc/rfc9000.html#name-connection_close-frames
      offset++;
      quic_get_variable_length(start,offset);
...
void QUIC::quic_copy_crypto(uint8_t * start , uint64_t &offset)
{
    offset += 1;
...
bool QUIC::quic_reassemble_frames()
{
   ...
   uint8_t * current = decrypted_payload + offset;
    ...
    while (quic_check_pointer_pos(current,payload_end)) 
    {
      ...
      if (quic_check_frame_type(decrypted_payload + offset,CRYPTO)) 
      {
         quic_copy_crypto(decrypted_payload,offset); 
      } 
      else if (quic_check_frame_type(decrypted_payload + offset,ACK1))
      {
         quic_skip_ack1(decrypted_payload,offset);
      } 
      else if (quic_check_frame_type(decrypted_payload + offset,ACK2))
      {
         quic_skip_ack1(decrypted_payload,offset);
      } 
      else if (quic_check_frame_type(decrypted_payload + offset,CONNECTION_CLOSE1))
      {
          quic_skip_connection_close1(decrypted_payload,offset);
      } 
      else if (quic_check_frame_type(decrypted_payload + offset,CONNECTION_CLOSE2))
      {
         quic_skip_connection_close2(decrypted_payload,offset);
      } 
      else if (quic_check_frame_type(decrypted_payload + offset,PADDING) ||
               quic_check_frame_type(decrypted_payload + offset,PING))
      {
         offset++;
      }
      else
...
void QUIC::quic_initialze_arrays()
{
...
bool QUIC::quic_check_initial(uint8_t packet0)
{
...
bool QUIC::quic_initial_checks(const Packet&pkt)
{
...
bool QUIC::quic_parse_header(const Packet & pkt)
{  
    uint8_t *payload_pointer = pkt.payload;
    uint64_t offset = 0;
...
bool QUIC::quic_start(const Packet& pkt)
{
