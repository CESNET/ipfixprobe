 ...
 */
class FlowHashPlugin : public ProcessPlugin {
public:
	 ...
	 *
	 * Constructs `FlowHashData` in `pluginContext` and initializes it with
	...
	/**
	 * @brief Cleans up and destroys `FlowHashData`.
	 * @param pluginContext Pointer to `FlowHashData`.
	 */
	void onDestroy(void* pluginContext) override;

	/**
	 * @brief Provides the memory layout of `FlowHashData`.
