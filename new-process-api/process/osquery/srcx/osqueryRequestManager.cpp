...
std::optional<JsonParser::AboutProgram> OSQueryRequestManager::readInfoAboutProgram(const FlowKey& flowKey) noexcept
{
	...
	const std::string query
		= std::format("SELECT p.name, u.username FROM processes AS p INNER JOIN users AS u ON p.uid=u.uid "
		  "WHERE p.pid='{}';\r\n", *pid);
...
OSQueryRequestManager::executeQuery(std::string_view query, const bool reopenFileDescriptor) noexcept
{
	...
	const std::optional<boost::static_string<BUFFER_SIZE>> queryResult
        = readFromOsquery();
...
{
    const std::string srcIp = ntohIPAddress(flowKey.srcIp).toString();
    const std::string dstIp = ntohIPAddress(flowKey.dstIp).toString();
    ...
    const std::optional<std::string_view> pid = JsonParser::findValueByKey(
        std::string_view(queryResult->data(), queryResult->size()), "pid");
    if (!pid.has_value()) {
        return std::nullopt;
    }
