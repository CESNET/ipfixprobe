...
constexpr static
std::pair<std::string_view, ParsingStatus> parseString(std::string_view data) noexcept
{
	bool quotesSeen = false;
	for (std::size_t tokenLength = 0; tokenLength < data.size(); tokenLength++) {
        switch (data[tokenLength])
        {
        case 0: return {"", ParsingStatus::FAILURE};
        case '}': return {"", ParsingStatus::NO_DATA};
        case '\"':{
            if (!quotesSeen) {
                quotesSeen = true;
                continue;
...
constexpr static
ParsedJsonItem parseJsonItem(std::string_view data) noexcept
{
    ParsedJsonItem res;
	...
	if (valueParsingStatus == ParsingStatus::FAILURE || valueParsingStatus == ParsingStatus::NO_DATA) {
		res.status = ParsingStatus::FAILURE;
		return res;
	}
    res.value = value;
...
constexpr static
std::optional<std::size_t> findParsingStartPosition(std::string_view data) noexcept
{
...
static auto
makeJsonItemReader(std::string_view data) noexcept
{
...
std::optional<std::string_view> JsonParser::findValueByKey(std::string_view data, std::string_view key)
{
...
constexpr static
bool setKeyToValue(auto& keysMapping, auto& keysParsed,
    std::string_view key, std::string_view value)
{
    auto it = std::find_if(keysMapping.begin(), keysMapping.end(),
...
struct AboutProgramMapping
{
    ...
    : keysMapping({std::make_pair("name"sv, std::ref(aboutProgram.name)),
...
constexpr static
std::optional<AboutType> parseAboutData(std::string_view jsonData) noexcept
{
    ...
    for (const ParsedJsonItem item : makeJsonItemReader(jsonData.substr(*startingPos))) {
        if (item.status == ParsingStatus::NO_DATA) {
            return std::all_of(mapping.keysParsed.begin(), mapping.keysParsed.end(),
                [](bool parsed) { return parsed; }) ? aboutData : std::nullopt;
        }
...
std::optional<JsonParser::AboutProgram> JsonParser::parseJsonAboutProgram(std::string_view jsonData) noexcept
{
...
struct AboutOSVersionMapping
{
    ...
    : keysMapping({std::make_pair("arch"sv, std::ref(aboutOSVersion.arch)),
...
std::optional<JsonParser::AboutOSVersion> JsonParser::parseJsonOSVersion(std::string_view jsonData) noexcept
{
