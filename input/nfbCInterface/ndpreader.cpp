...
int NdpReader::init_interface(const std::string& interface)
{
    ...
    std::size_t del_found_comma = p_interface.find_last_of(",");
    if (del_found_comma != std::string::npos) {
        p_interface2 = p_interface.substr(del_found_comma + 1);
        p_interface = p_interface.substr(0, del_found_comma);
    }

    if (p_interface2.size() > 0) {
        dev_handle = nfb_open(p_interface2.c_str());
        if (!dev_handle) {
            error_msg = std::string() + "unable to open NFB device '" + p_interface2 + "'";
            return 1;
        }
        ...
        rx_handles.push_back(rx_handle);
    }
...
bool NdpReader::retrieve_ndp_packets()
{
    ...
    if (ndp_packet_buffer_valid) {
        ndp_rx_burst_put(rx_handles[port_index % rx_handles.size()]);
        port_index++;
        ndp_packet_buffer_valid = false;
    }
    ret = ndp_rx_burst_get(
        rx_handles[port_index % rx_handles.size()],
        ndp_packet_buffer,
        packet_bufferSize);
    if (ret > 0) {
        ndp_packet_buffer_processed = 0;
