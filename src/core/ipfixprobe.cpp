...
#define PACKAGE_NAME "ipfixprobe"
...
bool process_plugin_args(ipxp_conf_t &conf, IpfixprobeOptParser &parser)
{
...

   {
      ...
      for (auto& manifest: inputFactory.getRegisteredPlugins()) {
   ...
   for (auto &it : parser.m_input) {
      std::unique_ptr<InputPlugin> inputPlugin = nullptr;
      std::unique_ptr<StoragePlugin> storagePlugin = nullptr;
      ...
      auto& inputFactory = InputPluginFactory::getInstance();
      auto& processFactory = ProcessPluginFactory::getInstance();

      
      //mgr.load1("/usr/local/lib64/ipfixprobe/input/libipfixprobe-input-raw.so");
      ...
      std::cout << "Process plugins: " << std::endl;

      for (auto& manifest: processFactory.getRegisteredPlugins()) {
         //std::cout << "Manifest: " << manifest.name << std::endl;
         //std::cout << "----------------\n";
      }

      try {
         inputPlugin = inputFactory.createUnique(input_name, input_params);
      ...
      input_plugin = inputPlugin.get();

      conf.m_inputPlugins.push_back(std::move(inputPlugin));
      ...
      */
      ...
      for (auto& manifest: storageFactory.getRegisteredPlugins()) {
         std::cout << "Manifest storage: " << manifest.name << std::endl;
         std::cout << "----------------\n";
      }

      try {
         storagePlugin = storageFactory.createUnique(storage_name, storage_params, output_queue);
      ...
      storage_plugin = storagePlugin.get();
      conf.m_storagePlugins.push_back(std::move(storagePlugin));
