...
constexpr static bool alwaysMatcher(std::string_view, std::string_view) noexcept { return true; }

constexpr static bool neverMatcher(std::string_view, std::string_view) noexcept { return false; }

static std::unordered_map<std::string, std::vector<std::string>> parseWhitelist(std::string_view filename)
{
...
static bool listMatcher(const std::unordered_map<std::string, std::vector<std::string>>& whitelist, std::string_view service, std::string_view txtValue)
{
...
ServiceFilter::ServiceFilter(const DNSSDOptionsParser::TxtProcessingMode& mode)
{
    ...
    m_matcher = [whitelist = parseWhitelist(std::get<std::string>(*mode))](std::string_view service, std::string_view txtValue) {
        return listMatcher(whitelist, service, txtValue);
    };
}
