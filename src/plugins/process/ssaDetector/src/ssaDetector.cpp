	...
	FieldHandlers<SSADetectorFields> handlers) noexcept
{
	...
	handlers.insert(
		...
		schema.addScalarField("SSA_CONF_LEVEL", [](const void* context) {
			return reinterpret_cast<const SSADetectorData*>(context)->confidence;
		}));
	return schema;
}
	...
	SSADetectorData& pluginData) noexcept
{
	...
	const bool foundTCPHandshake
		= pluginData.processingState.synAckPackets.hasSimilarPacketsRecently(
			length,
	...
	if (foundTCPHandshake) {
		...
		if (pluginData.processingState.suspectLengths.size()
			!= pluginData.processingState.suspectLengths.capacity()) {
	...
	const bool correspondingSynFound
		= pluginData.processingState.synPackets.hasSimilarPacketsRecently(
			length,
