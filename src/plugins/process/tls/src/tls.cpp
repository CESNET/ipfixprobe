...
createTLSSchema(FieldManager& fieldManager, FieldHandlers<TLSFields>& handlers) noexcept
{
	...
	handlers.insert(TLSFields::TLS_SNI, schema.addScalarField("TLS_SNI", [](const void* context) {
		return toStringView(reinterpret_cast<const TLSData*>(context)->serverNames);
	}));
	// TODO FIX
	/*handlers.insert(TLSFields::TLS_JA3, schema.addVectorField(
		"TLS_JA3",
		[](const void* context) { return toSpan<const uint8_t>(reinterpret_cast<const
	TLSData*>(context)->ja3); }
	));*/
	handlers.insert(TLSFields::TLS_JA4, schema.addScalarField("TLS_JA4", [](const void* context) {
		return toStringView(reinterpret_cast<const TLSData*>(context)->ja4);
	}));
	handlers.insert(TLSFields::TLS_ALPN, schema.addScalarField("TLS_ALPN", [](const void* context) {
		return toStringView(reinterpret_cast<const TLSData*>(context)->serverALPNs);
	}));
	handlers.insert(
		TLSFields::TLS_VERSION,
		schema.addScalarField("TLS_VERSION", [](const void* context) {
			return reinterpret_cast<const TLSData*>(context)->version;
		}));
	/*handlers.insert(TLSFields::TLS_EXT_TYPE, schema.addVectorField(
		"TLS_EXT_TYPE",
		[](const void* context) { return toSpan<const uint16_t>(reinterpret_cast<const
	TLSData*>(context)->extensionTypes); }
	));
	handlers.insert(TLSFields::TLS_EXT_LEN, schema.addVectorField(
		"TLS_EXT_LEN",
		[](const void* context) { return toSpan<const uint16_t>(reinterpret_cast<const
	TLSData*>(context)->extensionLengths); }
	));*/
...
{
	auto payload = getPayload(flowContext.packet);
	TLSParser parser;
...
PluginUpdateResult TLSPlugin::onUpdate(const FlowContext& flowContext, void* pluginContext)
{
	...
	if (pluginData->processingState.serverHelloParsed
		&& pluginData->processingState.clientHelloParsed) {
		return {
	...
	FlowRecord& flowRecord) noexcept
{
	return parser.parseExtensions([&](const TLSExtension& extension) {
		...
		case TLSExtensionType::SUPPORTED_GROUPS: {
			pluginData.processingState.supportedGroups
		...
		case TLSExtensionType::ELLIPTIC_CURVE_POINT_FORMATS: {
			pluginData.processingState.pointFormats
...
void TLSPlugin::saveJA3(
	...
	FlowRecord& flowRecord) noexcept
{
	if (!parser.cipherSuites.has_value() || !pluginData.processingState.supportedGroups.has_value()
	...
	FlowRecord& flowRecord) noexcept
{
	if (!pluginData.processingState.alpns.has_value()
		...
		|| !parser.cipherSuites.has_value() || !pluginData.processingState.serverNames.has_value()
