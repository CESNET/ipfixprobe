...
constexpr static inline auto split(std::string_view view, const char delimiter) noexcept
{
	return Generator::generate([&view, delimiter]() mutable {
			   ...
			   const auto token
				   = std::make_optional<std::string_view>(view.substr(0, delimiterPos));

			   view.remove_prefix(delimiterPos + 1);

			   return token;
		   })
		| std::views::take_while([](const auto& token) { return token.has_value(); })
		| std::views::transform([](const auto& token) { return *token; });
}
