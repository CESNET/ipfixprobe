...
struct HeaderFieldReader : public RangeReader {
	...
	{
		return Generator::generate([this, payload]() mutable -> std::optional<HeaderField> {
			...
			| std::views::take_while(
				   [](const std::optional<HeaderField>& v) { return v.has_value(); })
			| std::views::transform([](const std::optional<HeaderField>& v) { return *v; });
	}
};
