...
int RecordExtPHISTS::REGISTERED_ID = register_extension();
...
PHISTSPlugin::PHISTSPlugin(const std::string& params) : use_zeros(false)
{
   init(params.c_str());
}
...
static const PluginRegistrar<PHISTSPlugin, ProcessPluginFactory> phistsRegistrar(phistsPluginManifest);
