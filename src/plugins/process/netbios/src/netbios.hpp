...
public:
	 ...
	 *
	 * Constructs `NetBIOSData` in `pluginContext` and initializes it with
	...
	/**
	 * @brief Cleans up and destroys `NetBIOSData`.
	 * @param pluginContext Pointer to `NetBIOSData`.
	 */
	void onDestroy(void* pluginContext) override;

	/**
	 * @brief Provides the memory layout of `NetBIOSData`.
