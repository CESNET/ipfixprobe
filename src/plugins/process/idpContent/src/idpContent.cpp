	...
	IDPContentContext& idpContext) noexcept
{
	...
	if (idpContext.directionalContent[features.direction].has_value()) {
		return !idpContext.directionalContent[!features.direction].has_value();
	}
	...
	const std::size_t sizeToSave
		= std::min<size_t>(IDPContentContext::MAX_CONTENT_LENGTH, features.ipPayloadLength);
	std::span<const std::byte> payload = getPayload(packet);
	idpContext.directionalContent[features.direction]
		= std::make_optional<IDPContentContext::Content>(
			payload.data(),
