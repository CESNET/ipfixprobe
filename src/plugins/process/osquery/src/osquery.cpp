...
int RecordExtOSQUERY::REGISTERED_ID = register_extension();
...
OSQUERYPlugin::OSQUERYPlugin(const std::string& params) : manager(nullptr), numberOfSuccessfullyRequests(0)
{
   init(params.c_str());
}
...
static const PluginRegistrar<OSQUERYPlugin, ProcessPluginFactory> osqueryRegistrar(osqueryPluginManifest);
