...
createMPLSSchema(FieldManager& fieldManager, FieldHandlers<MPLSFields>& handlers) noexcept
{
	...
	/*handlers.insert(
		...
		schema.addVectorField("MPLS_TOP_LABEL_STACK_SECTION", getMPLSTopLabelStackSectionField));*/
...
{
	auto mplsView = flowContext.packetContext.packet->getLayerView<amon::layers::MPLSView>(
	...
	std::construct_at(reinterpret_cast<MPLSContext*>(pluginContext))->topLabel = mplsView->label();
