...
int RecordExtPSTATS::REGISTERED_ID = register_extension();
...
PSTATSPlugin::PSTATSPlugin(const std::string& params) : use_zeros(false), skip_dup_pkts(false)
{
   init(params.c_str());
}
...
static const PluginRegistrar<PSTATSPlugin, ProcessPluginFactory> pstatsRegistrar(pstatsPluginManifest);
