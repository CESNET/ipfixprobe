...
FieldGroup createQUICSchema(FieldManager& fieldManager, FieldHandlers<QUICFields>& handlers) noexcept
{
	...
	handlers.insert(QUICFields::QUIC_SNI, schema.addScalarField(
		"QUIC_SNI", 
		[](const void* context) { return toStringView(static_cast<const QUICData*>(context)->serverName);
	}));
	handlers.insert(QUICFields::QUIC_USER_AGENT, schema.addScalarField(
		"QUIC_USER_AGENT", 
		[](const void* context) { return toStringView(static_cast<const QUICData*>(context)->userAgent);
	}));
	handlers.insert(QUICFields::QUIC_VERSION, schema.addScalarField(
		"QUIC_VERSION", 
		[](const void* context) { return static_cast<const QUICData*>(context)->quicVersion; }
	));
	...
	handlers.insert(QUICFields::QUIC_TOKEN_LENGTH, schema.addScalarField(
		"QUIC_TOKEN_LENGTH",
		[](const void* context) { return static_cast<const QUICData*>(context)->quicTokenLength; }
	));
	handlers.insert(QUICFields::QUIC_OCCID, schema.addScalarField(
		"QUIC_OCCID", 
		[](const void* context) { return toStringView(static_cast<const QUICData*>(context)->originalClientId); }
	));
	handlers.insert(QUICFields::QUIC_OSCID, schema.addScalarField(
		"QUIC_OSCID", 
		[](const void* context) { return toStringView(static_cast<const QUICData*>(context)->originalServerId); }
	));
	handlers.insert(QUICFields::QUIC_SCID, schema.addScalarField(
		"QUIC_SCID", 
		[](const void* context) { return toStringView(static_cast<const QUICData*>(context)->sourceId); }
	));
	handlers.insert(QUICFields::QUIC_RETRY_SCID, schema.addScalarField(
		"QUIC_RETRY_SCID", 
		[](const void* context) { return toStringView(static_cast<const QUICData*>(context)->retrySourceId); }
	));
	handlers.insert(QUICFields::QUIC_MULTIPLEXED, schema.addScalarField(
		"QUIC_MULTIPLEXED", 
		[](const void* context) { return static_cast<const QUICData*>(context)->multiplexedCount; }
	));
...
//	????	m_exportData.retrySourceId, m_exportData.destinationConnectionId)) {
			pluginData.retrySourceId, initialHeaderView.destinationConnectionId)) {
		return;
	}

	const bool hasMultiplexing =
		(std::ranges::equal(
			pluginData.originalServerId, initialHeaderView.destinationConnectionId) ||
			std::ranges::equal(
//		????	m_exportData.sourceCID, initialHeaderView.destinationConnectionId)) &&
	...
	if (hasMultiplexing) {
		pluginData.multiplexedCount = std::min<uint16_t>(
			pluginData.multiplexedCount + 1, std::numeric_limits<uint8_t>::max());
		return;
	}
	...
	copyFromIfNotEmptyTo(
		initialHeaderView.destinationConnectionId, pluginData.originalServerId);
	copyFromIfNotEmptyTo(
		initialHeaderView.sourceConnectionId, pluginData.originalClientId);

	copyFromIfNotEmptyTo(
		initialHeaderView.extensionTypes, pluginData.tlsExtensionTypes);
	copyFromIfNotEmptyTo(
		initialHeaderView.extensionLengths, pluginData.tlsExtensionLengths);
...
) noexcept
{
	...
	*/
	...
	pluginData.retrySourceId.assign(
...
) noexcept
{
	...
	tryToSetOCCIDandSCID(
		*quicDirection, sourceConnectionId, destinationConnectionId, pluginData);
	return true;
}
