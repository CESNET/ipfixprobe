...
void OpenVPNProcessingState::processHardResetFromClient(const IPAddress& srcIp) noexcept
{
...
void OpenVPNProcessingState::processHardResetFromServer(const IPAddress& dstIp) noexcept
{
...
void OpenVPNProcessingState::processControl(const IPAddress& srcIp, 
    const IPAddress& dstIp, const bool hasTLSClientHello) noexcept
{
    ...
    if (m_state == State::CLIENT_HELLO && m_clientIp == dstIp && hasTLSClientHello) { // server to client
...
void OpenVPNProcessingState::processAck(const IPAddress& srcIp) noexcept
{
...
void OpenVPNProcessingState::processData(const std::size_t packetLength,
...
void OpenVPNProcessingState::processOpcode(const OpenVPNOpcode opcode, 
    const IPAddress& srcIp, const IPAddress& dstIp, const bool hasTLSClientHello, 
    const bool isValidRTPHeader, const std::size_t packetLength) noexcept
{
    m_invalidPacketCount++;
    switch (opcode) {
	case OpenVPNOpcode::P_CONTROL_HARD_RESET_CLIENT_V1: [[fallthrough]];
	case OpenVPNOpcode::P_CONTROL_HARD_RESET_CLIENT_V2: [[fallthrough]];
	case OpenVPNOpcode::P_CONTROL_HARD_RESET_CLIENT_V3:
        processHardResetFromClient(srcIp);
		break;

	case OpenVPNOpcode::P_CONTROL_HARD_RESET_SERVER_V1: [[fallthrough]];
	...
	case OpenVPNOpcode::P_DATA_V1: [[fallthrough]];
...
OpenVPNProcessingState::getCurrentConfidenceLevel(const std::size_t packetsTotal) const noexcept
{
    ...
    if (m_largePacketCount > MIN_PACKETS_IN_FLOW
		&& (static_cast<double>(m_dataPacketCount) 
                / static_cast<double>(m_largePacketCount) 
            >= LARGE_DATA_PACKET_RATIO)) {
		return static_cast<uint8_t>(
            (static_cast<double>(m_dataPacketCount) 
                / static_cast<double>(m_largePacketCount) * 80));
	}
