...
createSIPSchema(FieldManager& fieldManager, FieldHandlers<SIPFields>& handlers) noexcept
{
	...
	handlers.insert(
		...
		schema.addScalarField("SIP_MSG_TYPE", [](const void* context) {
			return reinterpret_cast<const SIPData*>(context)->messageType;
		}));
	handlers.insert(
		SIPFields::SIP_STATUS_CODE,
		schema.addScalarField("SIP_STATUS_CODE", [](const void* context) {
			return reinterpret_cast<const SIPData*>(context)->statusCode;
		}));
	handlers.insert(SIPFields::SIP_CSEQ, schema.addScalarField("SIP_CSEQ", [](const void* context) {
		return toStringView(reinterpret_cast<const SIPData*>(context)->commandSequence);
	}));
	handlers.insert(
		SIPFields::SIP_CALLING_PARTY,
		schema.addScalarField("SIP_CALLING_PARTY", [](const void* context) {
			return toStringView(reinterpret_cast<const SIPData*>(context)->callingParty);
		}));
	handlers.insert(
		SIPFields::SIP_CALLED_PARTY,
		schema.addScalarField("SIP_CALLED_PARTY", [](const void* context) {
			return toStringView(reinterpret_cast<const SIPData*>(context)->calledParty);
		}));
	handlers.insert(
		SIPFields::SIP_CALL_ID,
		schema.addScalarField("SIP_CALL_ID", [](const void* context) {
			return toStringView(reinterpret_cast<const SIPData*>(context)->callId);
		}));
	handlers.insert(
		SIPFields::SIP_USER_AGENT,
		schema.addScalarField("SIP_USER_AGENT", [](const void* context) {
			return toStringView(reinterpret_cast<const SIPData*>(context)->userAgent);
		}));
	handlers.insert(
		SIPFields::SIP_REQUEST_URI,
		schema.addScalarField("SIP_REQUEST_URI", [](const void* context) {
			return toStringView(reinterpret_cast<const SIPData*>(context)->requestURI);
		}));
	handlers.insert(SIPFields::SIP_VIA, schema.addScalarField("SIP_VIA", [](const void* context) {
		return toStringView(reinterpret_cast<const SIPData*>(context)->via);
	}));
...
constexpr bool SIPPlugin::parseSIPData(
	...
	FlowRecord& flowRecord) noexcept
{
	...
	if (pluginData.messageType <= 10) {
		...
		std::ranges::copy(
			tokens[1] | std::views::take(pluginData.requestURI.capacity()),
	...
	if (static_cast<SIPMessageType>(pluginData.messageType) == SIPMessageType::REPLY) {
		...
		if (std::from_chars(tokens[1].begin(), tokens[1].end(), pluginData.statusCode).ec
			== std::errc()) {
			return false;
		}
	...
	for (const auto& [key, value] : headerFieldReader.getRange(payload.substr(headerEnd + 1))) {
		if (key == "FROM" || key == "F") {
			...
			std::ranges::copy(
				getURI(value) | std::views::take(pluginData.callingParty.capacity()),
		...
		if (key == "TO" || key == "T") {
			...
			std::ranges::copy(
				getURI(value) | std::views::take(pluginData.calledParty.capacity()),
		...
		if (key == "CALL-ID" || key == "I") {
			...
			std::ranges::copy(
				value | std::views::take(pluginData.callId.capacity()),
		...
		if (key == "USER-AGENT") {
			...
			std::ranges::copy(
				value | std::views::take(pluginData.userAgent.capacity()),
		...
		if (key == "CSeq") {
			...
			std::ranges::copy(
				value | std::views::take(pluginData.commandSequence.capacity()),
...
{
	if (getMessageType(toStringView(flowContext.packet.payload, flowContext.packet.payload_len))
			.has_value()) {
