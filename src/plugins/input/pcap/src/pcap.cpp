...
public:
      ...
      m_file(""), m_ifc(""), m_filter(""), m_snaplen(-1), m_id(0), m_list(false)
   {
      ...
      register_option("l", "list", "", "Print list of available interfaces", [this](const char *arg){(void) arg; m_list = true; return true;}, OptionFlags::NoArgument);
   }
};
...
PcapReader::PcapReader(const std::string& params) : m_handle(nullptr), m_snaplen(-1), m_datalink(0), m_live(false), m_netmask(PCAP_NETMASK_UNKNOWN)
{
   init(params.c_str());
}
...
void PcapReader::check_datalink()
{
...
OptionsParser *PcapReader::get_parser() const {
    return new PcapOptParser(); 
}
