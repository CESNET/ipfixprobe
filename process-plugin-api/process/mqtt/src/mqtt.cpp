...
FlowAction MQTTPlugin::onFlowCreate(FlowRecord& flowRecord, const Packet& packet)
{
	if (mqttLabelPresent(packet.payload)) {
		return parseMQTT(packet.payload, flowRecord);
	}
 ...
 */
FlowAction MQTTPlugin::parseMQTT(std::span<const std::byte> payload, FlowRecord& flowRecord) noexcept
{
	if (payload.empty()) {
		return FlowAction::RequestNoData;
	}
	...
	while (currentOffset < payload.size()) {
		...
		case MQTTHeaderType::CONNECT: {
			...
			m_exportData.version = static_cast<uint8_t>(payload[currentOffset++]);
			m_fieldHandlers[MQTTFields::MQTT_VERSION].setAsAvailable(flowRecord);

			// Only MQTT v3.1.1 and v5.0 are supported
			if (m_exportData.version != 4 && m_exportData.version != 5) {
				return FlowAction::RequestNoData;
			}
			m_exportData.connectionFlags = static_cast<uint8_t>(payload[currentOffset++]);
			m_fieldHandlers[MQTTFields::MQTT_CONNECTION_FLAGS].setAsAvailable(flowRecord);
			
			m_exportData.keepAlive 
		...
		case MQTTHeaderType::CONNECT_ACK: {
			...
			m_exportData.connectionReturnCode = static_cast<uint8_t>(payload[currentOffset++]);
			m_fieldHandlers[MQTTFields::MQTT_CONNECTION_RETURN_CODE].setAsAvailable(flowRecord);
			
			break;
		}
		case MQTTHeaderType::PUBLISH: {
			m_exportData.publishFlags |= typeFlag.bitfields.flag;
...
FlowAction MQTTPlugin::onFlowUpdate(FlowRecord& flowRecord, const Packet& packet)
{
	return parseMQTT(packet.payload, flowRecord);
}

ProcessPlugin* MQTTPlugin::clone(std::byte* constructAtAddress) const
{
	return std::construct_at(reinterpret_cast<MQTTPlugin*>(constructAtAddress), *this);
}

std::string MQTTPlugin::getName() const { 
	return mqttPluginManifest.name; 
}

const void* MQTTPlugin::getExportData() const noexcept {
	return &m_exportData;
}	
